...
// Provide fallback implementations when C++20 <bit> functions are unavailable.
// These use compiler builtins which are widely supported.
static inline int popcount(unsigned x) { return __builtin_popcount(x); }
static inline int countr_zero(unsigned x) { return __builtin_ctz(x); }
